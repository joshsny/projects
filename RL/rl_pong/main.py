...
learning_rate = 5e-4 #for convergence (too low- slow to converge, too high,never converge)
gamma = 0.99 # discount factor for reward (i.e later rewards are exponentially less important)
decay_rate = 0.99 # decay factor for RMSProp leaky sum of grad^2
resume = True # resume from previous checkpoint?
decision_rate = 1 # This only allows the AI to make a decision every x frames
model_save_name = 'save-dr' + str(decision_rate) + '.p'
# model initialization
D = 80 * 80 # input dimensionality: 80x80 grid (the pong world)
if resume:

    model = pickle.load(open('save-dr1.p', 'rb')) #load from pickled checkpoint
else:
...
#begin training!
while True:
    # If wanting to watch a game, implement sleep
    #sleep(.01)
    ...
    # This only allows the AI to make a decision every x frames
    if j % decision_rate == 0:
    ...
    if done: # an episode finished
        ...
        if episode_number % (batch_size*3) == 0:
